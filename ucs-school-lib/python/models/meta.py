...
class UCSSchoolHelperOptions(object):
    def __init__(self, klass, meta=None):
        ...
        if self.udm_module:
            ...
            ldap_unmap_function = partial(
                module.mapping.unmapValue, module.mapping.mapName(udm_name)
            )
...
class UCSSchoolHelperMetaClass(type):
    def __new__(mcs, cls_name, bases, attrs):
        ...
        cls = super(UCSSchoolHelperMetaClass, mcs).__new__(
            mcs, cls_name, bases, dict(attrs)
        )
